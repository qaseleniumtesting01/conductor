...
public class DeciderService {
    ...
    private final Predicate<Workflow> containsTerminalTerminateTask = workflow -> workflow.getTasks().stream()
    .anyMatch(task -> TERMINATE.name().equals(task.getTaskType()) && task.getStatus().isTerminal());
    ...
    private DeciderOutcome decide(final Workflow workflow, List<Task> preScheduledTasks)
        ...
        if (containsTerminalTerminateTask.test(workflow) || (outcome.tasksToBeScheduled.isEmpty() && checkForWorkflowCompletion(workflow))) {
     ...
     *                 are specified in the workflow definition if null, the output of the last task in the workflow will
    ...
    boolean checkForWorkflowCompletion(final Workflow workflow) throws TerminateWorkflowException {
        ...
        if (allTasks.stream()
            .anyMatch(task -> TaskType.TERMINATE.name().equals(task.getTaskType()) && task.getStatus().isTerminal())) {
            return true;
        }
